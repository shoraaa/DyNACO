...
public:
    ...
    std::vector<int32_t> source_positions; // (n,) position of customer in source_perm; depot=-1
...
public:
    ...
    void sample(bool require_prob, const float* residual_logits, SampleResult& result, bool parallel_traced);
    ...
    // perm_ptr length m
    void decode_perm_to_route0(const int32_t* perm_ptr, std::vector<int32_t>& out_route0) const;
...
private:
    ...
    float relocate_node(int32_t target, int32_t node, std::vector<int32_t>& perm, std::vector<int32_t>& positions);
    float two_opt_nn(std::vector<int32_t>& perm, std::vector<int32_t>& positions, std::vector<int32_t>& checklist);
    void flip_route_section(int32_t start_node, int32_t end_node, std::vector<int32_t>& perm, std::vector<int32_t>& positions);

    bool contains_edge(int32_t a, int32_t b) const;
    int32_t get_succ(int32_t node, const std::vector<int32_t>& perm, const std::vector<int32_t>& positions) const;
    int32_t get_pred(int32_t node, const std::vector<int32_t>& perm, const std::vector<int32_t>& positions) const;

    // ---- CVRP split DP cost + segments ----
    struct SplitResult {
        float cost;
        std::vector<std::pair<int32_t,int32_t>> segs; // inclusive [i,j] on perm indices
    };
