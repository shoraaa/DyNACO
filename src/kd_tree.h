...
public:
    ...
    // By default, KDTree uses TSPLIB-style rounded EUC_2D distances.
    // Some algorithms (e.g. MFACO training) require exact (non-rounded)
    // explicit Euclidean distances.
    bool round_distances_ = true;


    explicit KDTree(const std::vector<Point> &points, bool round_distances = true) {
        round_distances_ = round_distances;
        points_ = points;
    ...
    uint32_t nn_bottom_up(uint32_t point_idx) {
        ...
        while (true) {
            ...
            // handled properly.
            const double extra = round_distances_ ? 1.0 : 0.0;
            if (ball_in_bounds(p->bounds_, nn_target_pt_idx_, nn_dist_ + extra)) {
                break ;
            }
        }
    ...
    [[nodiscard]] double get_distance(size_t first_pt_idx, size_t second_pt_idx) const {
        ...
        if (round_distances_) {
            return static_cast<double>(lround(d));
        }
        return d;
    }
